...
\begin{itemize}
    \item Alerts can be set via the \texttt{/set\_reminder} route to remind users of critical tasks.
    \item Environmental thresholds such as temperature or humidity exceeding user-defined limits trigger alerts, displayed on the ESP32 LCD and logged in the system.
...
\subsection{Optimized Resource Utilization}
Efficient memory management techniques, such as dynamic JSON parsing and modular code design, reduced the load on the ESP32. Tasks were distributed across multiple modules for better resource allocation.
